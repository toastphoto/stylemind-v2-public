...
- The private workbench now has first-pass priority-action and batch-QA controls that map the current visual-review queue to safe existing actions such as explicit upload-vs-AI page asset choice, selected/multiple-page rendered QA, or manual TemplateSkill repair for object-gate failures.
...
1. Deepen deck-level visual-review and 95-readiness priority actions with safer TemplateSkill fallback/penalty handling.
